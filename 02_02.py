...
class DataSet:
    ...
    @staticmethod
    def parser_csv(task):
        res_data = DataSet.reader_csv(task.parametrs['filename']['val'])
    ...
    def __init__(self, task):
        ...
        self.vac_obj = DataSet.parser_csv(task)
...
class DynamicObjects:  
    def __init__(self, task, vac_obj):
        ...
        for vac in vac_obj:
            ...
            salary = ((float(vac.salary.salary_to)+float(vac.salary.salary_from)) *
                    dic_currency[vac.salary.salary_currency]['cost'])
            if (year in self.salByYear['val'].keys()):
                self.salByYear['val'][year] += salary
                self.vacByYear['val'][year] += 1
            else:
                self.salByYear['val'][year] = salary
                self.vacByYear['val'][year] = 1
            if (task.parametrs['prof']['val'] in vac.name):
                if (year in self.salByYearProf['val'].keys()):
                    self.salByYearProf['val'][year] += salary
            ...
            if (city in self.salByCity['val'].keys()):
                self.salByCity['val'][city] += salary
...
class Report:
    def __init__(self, dataset):
        self.salary_year_level = dataset.dynamics_objects.salByYear['val']
        ...
        self.salary_city_level = dataset.dynamics_objects.salByCity['val']
        self.vacancies_city_count = dataset.dynamics_objects.vacByCity['val']

    def generat_excel(self, prof):
        workbook_ = Workbook()
        stats_by_year = workbook_.worksheets[0]
        stats_by_year.title = "Cтатистика по годам"
        stats_by_city = workbook_.create_sheet("Cтатистика по городам")
        stats_by_year.append(["Год", "Средняя зарплата", f"Средняя зарплата - {prof}",
                              "Количество вакансий", f"Количество вакансий - {prof}"])
        for i, year in enumerate(self.salary_year_level.keys(), 2):
            stats_by_year.cell(row=i, column=1, value=year)
            for j, dictionary in enumerate((self.salary_year_level, self.vacancies_year_count, 
        ...
        for i, city in enumerate(self.salary_city_level.keys(), 2):
            stats_by_city.cell(row=i, column=1, value=city)
            stats_by_city.cell(row=i, column=2, value=self.salary_city_level[city])
    ...
    def generat_image(self, prof):
        fig1, ((f11, f12), (f21, f22)) = plt.subplots(2, 2, figsize=(12, 7.5), layout='constrained')
        self.generat_salByYear_graph(f11, prof)
        self.generat_vacByYear_graph(f12, prof)
        self.generat_salByCity_graph(f21)
        self.generat_vacByCity_graph(f22)
        plt.savefig('graph.png')

    def generat_salByYear_graph(self, f, prof):
        f_labels = self.salary_year_level.keys()
        x = np.arange(len(f_labels))
        width = 0.35
        f.bar(x - width / 2, self.salary_year_level.values(), width, label='Средняя з/п')
    ...
    def generat_vacByYear_graph(self, f, prof):
    ...
    def generat_salByCity_graph(self, f):
        f_labels = self.salary_city_level.keys()
        y_pos = np.arange(len(f_labels))
        f.barh(y_pos, self.salary_city_level.values(), align='center')
        f.set_yticks(y_pos, fontsize=6, labels=f_labels)
        f.invert_yaxis()
        f.xaxis.grid(True)
        f.set_title("Уровень зарплат по городам")

    def generat_vacByCity_graph(self, f):
...
report.generat_excel(my_task.parametrs['prof']['val'])
report.generat_image(my_task.parametrs['prof']['val'])